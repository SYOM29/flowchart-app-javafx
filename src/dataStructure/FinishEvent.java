...
public class FinishEvent implements Component
{
    ...
    private int objID;
    private final String imageRespath = "src\\images\\FinishEvent.png";
    private boolean isSelected;
    private String s;
    private Label FinishEventLabel;
    private Joint[] joints;
    private boolean readyToDelete;

    public FinishEvent(String s, double x, double y, int objID, Surec ownerProcess)
    {
        readyToDelete = false;
        ...
        FinishEventLabel = new Label("", take.getImageView());
        FinishEventLabel.setTranslateX(x);
        FinishEventLabel.setTranslateY(y);
        x = FinishEventLabel.getTranslateX();
        y = FinishEventLabel.getTranslateY();
        w = 70;
        h = 70;
        joints = new Joint[4];
        setJointLocations();
    ...
    {
        FinishEventLabel.setOnMouseDragged((MouseEvent event)
                ->
        {
            ...
            {
                moveWork(event.getX() + FinishEventLabel.getTranslateX() - w / 2, event.getY() + FinishEventLabel.getTranslateY() - h / 2);
                setTextCentered();
            }
        });
        FinishEventLabel.setOnMouseClicked((MouseEvent event)
                ->
        {
    ...
    {
        String s = "Finish Event number : " + objID;
        return s;
    }
    ...
    {
        info.setTranslateX(x + FinishEventLabel.getWidth() / 2 - info.getLabel().getWidth() / 2);
        info.setTranslateY(y + FinishEventLabel.getHeight() / 2 - info.getLabel().getHeight() / 2);
