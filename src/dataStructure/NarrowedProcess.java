...
public class NarrowedProcess implements Component
{
    ...
    private int objID;
    private final String imageRespath = "src\\images\\NarrowedProcess.png";
    private boolean isSelected;
    private String s;
    private Label narrowedProcessLabel;
    private Joint[] joints;
    private boolean readyToDelete;

    public NarrowedProcess(String s, double x, double y, int objID, Surec ownerProcess)
    {
        readyToDelete = false;
        ...
        narrowedProcessLabel = new Label("", take.getImageView());
        narrowedProcessLabel.setTranslateX(x);
        narrowedProcessLabel.setTranslateY(y);
        x = narrowedProcessLabel.getTranslateX();
        y = narrowedProcessLabel.getTranslateY();
        w = 130;
        h = 80;
        joints = new Joint[4];
        setJointLocations();
    ...
    {
        narrowedProcessLabel.setOnMouseDragged((MouseEvent event)
                ->
        {
            ...
            {
                moveWork(event.getX() + narrowedProcessLabel.getTranslateX() - w / 2, event.getY() + narrowedProcessLabel.getTranslateY() - h / 2);
                setTextCentered();
            }
        });
        narrowedProcessLabel.setOnMouseClicked((MouseEvent event)
                ->
        {
    ...
    {
        String s = "Narrowed Process number : " + objID;
        return s;
    }
    ...
    {
        info.setTranslateX(x + narrowedProcessLabel.getWidth() / 2 - info.getLabel().getWidth() / 2);
        info.setTranslateY(y + narrowedProcessLabel.getHeight() / 2 - info.getLabel().getHeight() / 2);
